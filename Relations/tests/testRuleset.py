...
class TestLibrary(CMFTestCase.CMFTestCase):
    ...
    def testActions(self):
        ...
        def hasaction(obj):
            ...
            if category not in at.listFilteredActions(obj).keys():
                return False
            for action in at.listFilteredActions(obj)[category]:
...
class TestRuleset(CMFTestCase.CMFTestCase):
    ...
    def testAllowedContentTypes(self):
        ...
        for ti in types:
            self.assert_(hasattr(ti, '_isTypeInformation'),
    ...
    def testRenameRulesetInLibrary(self):
        ...
        if HAVE_TS_MODULE:
            transaction.commit(1) # ensure object has a _p_jar
        else:
